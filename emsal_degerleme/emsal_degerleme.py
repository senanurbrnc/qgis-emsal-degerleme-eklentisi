...
class EmsalDegerleme:
    ...
    def duzeltme_grubu(self, feature, degisken):
        ...
        if degisken in ["sehir_manz", "doga_manz", "gol_manz", "bogaz_manz"]:
            return self.var_yok_grubu(self.sayi_al(feature, degisken, -1))
    ...
    def duzeltme_ortalamalari_hazirla(self, layer, hedef, ayar, emsaller=None):
        ...
        for degisken, etiket in self.duzeltme_degiskenleri():
            ...
            for f in baz_veri:
                ...
                gruplar[grup]["adet"] += 1
                gruplar[grup]["toplam"] += temel["guncel_m2"]
    ...
    def emsal_hesapla(self, f, hedef=None, duzeltme_ortalamalari=None):
        ...
        # Güncel hesap mantığı:
        # 1) Önce emsal satış fiyatı TÜFE ile değerleme tarihine güncellenir.
        # 2) Düzeltme tutarları güncel m² birim fiyat ortalamaları üzerinden hesaplanır.
        # 3) Toplam düzeltme tutarı TÜFE güncellenmiş fiyata eklenir/çıkarılır.
        tufe_duzeltilmis_fiyat = temel["duzeltilmis_fiyat"]
        ozellik_duzeltilmis_satis_fiyati = tufe_duzeltilmis_fiyat + toplam_duzeltme_tutari

        # Düzeltme sonrası fiyat sıfır veya negatif olursa hesap güvenilir değildir.
        if ozellik_duzeltilmis_satis_fiyati <= 0:
            return None

        tufe_guncellenmis_duzeltilmis_deger = ozellik_duzeltilmis_satis_fiyati
    ...
    def pdf_raporu_olustur(self, layer, hedef, ayar):
        ...
        # PDF raporunda en yakın en fazla 5 emsal gösterilir.
        # Hesaplama yine tüm hesaplanabilir emsaller üzerinden yapılır; bu sınır sadece PDF tablosunun görünümü içindir.
        ...
        row_defs = [
            ...
            ("Şehir Manzarası Düzeltmesi", "duz_sehir_manz"),
            ("Doğa Manzarası", "doga"),
            ("Doğa Manzarası Düzeltmesi", "duz_doga_manz"),
            ("Göl Manzarası", "gol"),
            ("Göl Manzarası Düzeltmesi", "duz_gol_manz"),
            ("Boğaz Manzarası", "bogaz"),
            ("Boğaz Manzarası Düzeltmesi", "duz_bogaz_manz"),
            ("Kuzey Cephe", "kuzey"),
            ("Güney Cephe", "guney"),
            ("Doğu Cephe", "dogu"),
            ("Batı Cephe", "bati"),
        ...
        # Dikey A4'te 5 emsalin sığabilmesi için kolon genişlikleri daha kompakt ayarlanır.
        # İlk kolon özellik adları için yeterli bırakılır; kalan alan konu taşınmaz ve emsaller arasında paylaştırılır.
        ozellik_genislik = 22
        veri_kolon_sayisi = 1 + len(rapor_emsalleri)  # konu taşınmaz + emsaller
        veri_genislik = (100 - ozellik_genislik) / veri_kolon_sayisi if veri_kolon_sayisi > 0 else 13

        header_cells = f'<th class="ozellik" width="{ozellik_genislik}%">Özellikleri</th><th width="{veri_genislik:.2f}%">Konu Taşınmaz</th>'
        for i in range(len(rapor_emsalleri)):
            header_cells += f'<th width="{veri_genislik:.2f}%">Emsal {i+1}</th>'
        ...
        def karsilastirma_tablosu_olustur(baslik, satirlar):
            ...
            # Dikey A4 raporda gereksiz boşluk kalmaması için hiçbir tablo
            # zorla yeni sayfaya atılmaz. CSS'teki page-break-inside: avoid ayarı
            # tablonun mümkün olduğunca bütün kalmasını sağlar; sayfada yer varsa
            # bir sonraki tablo aynı sayfada devam eder.
            ek_sinif = ''
...
'''
        ...
        # PDF sonunda, tahmini değer hesabında kullanılan tüm hesaplanabilir emsal değerleri ayrıca gösterilir.
        # Böylece tabloda en yakın 5 emsal yer alsa bile, ortalama hesabına giren tüm değerler raporda görülür.
        deger_metinleri = [f"{v:,.0f}" for v in degerler]
        toplam_deger = sum(degerler)
        pay_metni = " + ".join(deger_metinleri)
        nihai_hesap_html = f"""
<div class="explanation-box">
    Rapor tablosunda karşılaştırılabilirlik düzeyi en yüksek olan 5 emsale yer verilmiş, değer hesabında ise uygulama tarafından hesaplanabilir olarak belirlenen tüm emsal değerleri dikkate alınmıştır.
</div>

<div class="final-calc">
    <div class="final-title">Nihai Değer Hesabı</div>
    <div class="formula-line">Tahmini değer = ({pay_metni}) / {len(degerler)}</div>
    <div class="formula-line">Tahmini değer = {toplam_deger:,.0f} / {len(degerler)}</div>
    <div class="formula-result">Tahmini değer = {tahmini_deger:,.0f} TL</div>
</div>
"""
